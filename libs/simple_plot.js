class simplePlot {
  constructor(context, width, height, margin = 20, fsize = 12) {
    this.ctx = context;
    this.w = width;
    this.h = height;
    this.margin = margin;
    this.xScale = d3.scaleLinear().range([margin, this.w - margin]);
    this.yScale = d3.scaleLinear().range([this.h - margin, margin]);
    this.data = {};
    this.scatterData = {};
    this.range_x = null;
    this.range_y = null;
  ...
  scatter(label, x, y, marker = 'o', markercolor = '#000', markerradius = 1, markeredgewidth = 0, markeredgecolor = 'rgba(0,0,0,0)') {
    if (!this.scatterData.hasOwnProperty(label)) {
      this.scatterData[label] = {};
      this.scatterData[label].marker = marker;
      this.scatterData[label].markercolor = markercolor;
      this.scatterData[label].markerradius = markerradius;
      this.scatterData[label].markeredgewidth = markeredgewidth;
      this.scatterData[label].markeredgecolor = markeredgecolor;
    }
    ...
    if (!(this.range_y === null)) {
      ...
      this.rangeY([d3.min(y), d3.max(y)]);
    }

    this.draw();
  }

  plot(label, x, y, linedash = [], linecolor = '#000', linewidth = 1, is_step = false) {
  ...
  changeAttr(label, linedash = null, linecolor = null, linewidth = null, is_step = null) {
    if (linedash !== null)
      this.data[label].linedash = linedash;
    if (linecolor !== null)
      this.data[label].linecolor = linecolor;
  ...
  draw() {
    ...
    //console.log(metrics);
    //let fH = metrics.fontBoundingBoxAscent + metrics.fontBoundingBoxDescent;
    //console.log(fH);
    ...
    if (this.draw_legend) {
      ...
      let N_curves = Object.keys(this.data).length;
